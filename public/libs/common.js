...
function getTicket(api = null) {
  return axios
    .get(`/yz/referred/get-jsapi-ticket?url=${encodeURIComponent(window.location.href)}`)
    .then(res => {
...
function getReferred(id = '', customerPhone = '') {
  return axios
    .get(`/yz/referred/get-referred?id=${id}&customerPhone=${customerPhone}`)
    .then(r => r.data)
    .catch(err => console.log(err));
}
