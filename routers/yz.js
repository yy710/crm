...
module.exports = function (express) {
  ...
  routerAccessToken.use(accessToken.find(), accessToken.checkout(), accessToken.getNewToken(), accessToken.saveToDb());

  routerJsapiTicket.use(jsapiTicket.find(), jsapiTicket.checkout(), jsapiTicket.getNewTicket(), jsapiTicket.saveToDb());
  ...
  routerReferred.post('/msg', handleMsg(), mw.accept(), mw.dispatchPre(), mw.dispatch());
  ...
  routerReferred.get('/download', function (req, res, next) {
    ...
    req.data.db.collection('referreds').aggregate([
        { $match: { 'order.create_time': { $gte: startTime, $lte: endTime } } },
        { $sort: { _id: -1 } }
      ])
      .toArray()
      .then(r => {
  ...
  routerReferred.use('/delete-referred', (req, res, next) => {
    const { op, rfid } = req.query;
    req.data.db.collection('referreds').deleteOne({ id: rfid , 'order.dispatch_employer.id': 0}).then(r => {
      //assert.equal(1, r.deletedCount);
      if(r.deletedCount){
        res.json({ code: 1, msg: '订单已删除！' });
      }else{
        res.json({ code: 0, msg: '删除订单失败！' });
      }
    }).catch(err => console.log(err));
  });

  routerReferred.use('/getToken', routerAccessToken, (req, res, next) => {
  ...
  routerReferred.use('/get-referreds', async (req, res, next) => {
    try {
      ...
      if (global.config.debug) console.log('pipe: ', JSON.stringify(pipe, null, 4));
      const referreds = await col.aggregate(pipe).toArray();
      if (global.config.debug) isAdmin = true;
